...
const EnquiryForm = ({ source = 'contact' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Fallback: if EmailJS fails, open WhatsApp so lead is never lost
      toast.error(
        error.message ||
        'Something went wrong. Redirecting to WhatsApp...'
      );
      setTimeout(() => {
        const msg = encodeURIComponent(
          `Hi CASAMANDUVA! I'm ${formData.name.trim()}. ` +
          `I tried filling your form but had an issue. ` +
          `${formData.message?.trim() || ''}`
        );
        window.open(`https://wa.me/919121885090?text=${msg}`, '_blank');
      }, 1500);
    } finally {
  ...
  const openWhatsApp = () => {
    ...
    const message = encodeURIComponent(
      `Hi CASAMANDUVA! I'm ${formData.name.trim()}. ` +
      `Looking for ${formData.service || 'interior design'}. ` +
      `${formData.message?.trim() || ''}`
    );
      ...
      <form className="contact-form" onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-group">
          <label htmlFor="message">
            ...
            {errors.message && (
              <span style={{ color: 'var(--color-error)', fontWeight: '600', marginLeft: '0.5rem' }}>
                - {errors.message}
              </span>
            )}
          </label>
        ...
        <div className="form-actions">
          ...
          <button type="button" className="btn btn-outline" onClick={openWhatsApp} disabled={isSubmitting}>
